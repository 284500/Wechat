<template>
	<view>
		...
		<!-- 通讯录列表 -->
		<my-list-item v-for="item in mailList" @click="click(item.event)" :title="item.title"
			:img-url="item.imgUrl"></my-list-item>
		<view v-for="(item,index) in list" v-if="item.data.length>0">
			<view class="py-2 px-3 border-bottom bg-light">
				<text class="text-dark font-md">{{item.letter}}</text>
			</view>
			<my-list-item v-for="(item2,index2) in item.data" @click="click()" :title="item2"
